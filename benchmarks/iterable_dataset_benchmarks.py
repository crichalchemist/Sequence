...
class DatasetBenchmark:
    ...
    def benchmark_standard_dataset(self, data_cfg: DataConfig, feature_df: pd.DataFrame) -> Tuple[DataLoader, Dict[str, float]]:
        """Benchmark standard Dataset approach."""
        from data.agents.base_agent import SequenceDataset
    ...
    def benchmark_dataset_sizes(self, sizes: List[int]) -> Dict[int, Dict[str, Dict[str, float]]]:
        ...
        for size in sizes:
            ...
            try:
                ...
                print(f"  Iterable Dataset: {iter_stats['memory_used_mb']:.1f} MB, {iter_stats['execution_time_s']:.2f}s")
            except Exception as e:
    ...
    def benchmark_chunksize_impact(self) -> Dict[str, Dict[str, float]]:
        ...
        for chunksize in chunk_sizes:
            ...
            with patch.object(IterableFXDataset, '_find_cached_features', mock_find_cached_features):
                ...
                print(f"  Processed {count} samples in {stats['execution_time_s']:.2f}s using {stats['memory_used_mb']:.1f} MB")
    ...
    def generate_report(self, size_results: Dict, chunksize_results: Dict) -> str:
        ...
        for size in sorted(size_results.keys()):
            ...
            if 'error' not in std_result and 'error' not in iter_result:
                ...
                report.append(f"| {size:,} | {std_mem:.1f}, {std_time:.2f} | {iter_mem:.1f}, {iter_time:.2f} | {memory_savings:.1f}% | {speed_ratio:.2f}x |")
            else:

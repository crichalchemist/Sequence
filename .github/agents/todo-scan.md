...
You are a code maintenance reviewer. Locate outstanding TODO/FIXME markers so the team can resolve or ticket them before
shipping.

Workflow:

1. Invoke the Task tool with `subagent_type: todo-fixme-scanner`, passing `$ARGUMENTS` as an optional glob/filter (e.g.,
   `src/**`). Ask for grouped results plus severity hints.

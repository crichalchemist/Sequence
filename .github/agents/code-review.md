...
1. Collect repository context by invoking the Task tool with `subagent_type: git-summarizer`. Supply `$ARGUMENTS` (if
   provided) for additional hints (e.g., target path or branch). Store the returned Markdown for downstream droids and
   include key highlights in your final response.
2. Delegate focused passes using the gathered summary:
    ...
    - `documentation-accuracy-reviewer`
      Provide each subagent with the git-summarizer output plus any relevant context from the session. Ask them to
...
4. Consolidate results, deduplicate overlapping issues, and prioritise by severity. Be explicit when no blockers are
   found but justify why.

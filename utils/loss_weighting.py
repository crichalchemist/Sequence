...
class UncertaintyLossWeighting(nn.Module):
    ...
    def __init__(
        ...
        for task_name in task_names:
            if initial_weights and task_name in initial_weights:
                initial_log_var = torch.tensor(
                    torch.log(torch.tensor(initial_weights[task_name], dtype=torch.float32)), 
                    dtype=torch.float32
                )
            else:
    ...
    def get_total_weighted_loss(self, losses: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, Dict[str, float], torch.Tensor]:
